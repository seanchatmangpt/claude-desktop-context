...
echo "🔍 TRACE PROPAGATION ANALYSIS"
echo "============================="
echo ""

# Analyze the issue
echo "📊 Current State:"
echo "- All spans have the same trace ID ✅"
echo "- All spans have valid IDs ✅" 
echo "- Parent span IDs are all empty ❌"
echo ""

echo "🐛 Root Cause:"
echo "The parentSpanId field is always empty because:"
echo "1. GLOBAL_SPAN_ID is not being set correctly"
echo "2. Parent context is lost between function calls"
echo ""

echo "🔧 Solution:"
cat > /Users/sac/claude-desktop-context/telemetry/otel_lib_fixed.sh << 'EOF'
#!/bin/bash
...
echo "✅ Fixed library created"
echo ""

echo "🧪 Testing Fixed Implementation:"
source /Users/sac/claude-desktop-context/telemetry/otel_lib_fixed.sh

# Clear test data
rm -f "$TELEMETRY_DIR/data/traces_fixed.jsonl"
touch "$TELEMETRY_DIR/data/traces_fixed.jsonl"

# Test
otel_init "test"
trace=$(otel_start_trace "root.operation")
echo "Root trace: $trace"

span1=$(otel_start_span "child.1")
echo "Child 1: $span1 (parent: $OTEL_PARENT_SPAN_ID)"

span2=$(otel_start_span "grandchild.1.1")
echo "Grandchild: $span2 (parent: $OTEL_PARENT_SPAN_ID)"
otel_end_span

otel_end_span

span3=$(otel_start_span "child.2")
echo "Child 2: $span3 (parent: $OTEL_PARENT_SPAN_ID)"
otel_end_span
...
jq -r '.resourceSpans[0].scopeSpans[0].spans[0] | 
  if .parentSpanId == "" then 
    "├─ ROOT: \(.name) [\(.spanId[0:8])]"
  else 
    "│  └─ \(.name) [\(.spanId[0:8])] → parent: \(.parentSpanId[0:8])"
  end' "$TELEMETRY_DIR/data/traces_fixed.jsonl"

echo ""
total=$(wc -l < "$TELEMETRY_DIR/data/traces_fixed.jsonl")
with_parents=$(grep -c '"parentSpanId": "[a-f0-9]' "$TELEMETRY_DIR/data/traces_fixed.jsonl" || echo 0)
echo "Total spans: $total"
echo "Spans with parents: $with_parents"
echo "Parent tracking: $(( with_parents * 100 / (total - 1 + 1) ))%"